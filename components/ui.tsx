...
// --- Helper Functions (Moved to Top for Usage in Components) ---
...
export const Input = ({ label, error, suffix, showNumberHelper, className = "", value, ...props }: InputProps) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {/* Number Helper: Persian Words + 3-Digit Separator */}
      {shouldShowHelper && (
        <div className="flex justify-between items-start gap-2 px-1">
          <span className="text-xs text-brand-600 font-medium leading-tight">
             {numberToPersianWords(numVal)}
          </span>
          <span className="text-xs text-slate-500 font-mono dir-ltr tracking-wider">
             {formatNumber(numVal)}
          </span>
        </div>
      )}
